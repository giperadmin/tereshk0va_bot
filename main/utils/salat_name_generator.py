...
import utils.work_with_json as work_with_json
...
async def salat_name_generator(composition: str, state: FSMContext = None) -> dict:
    ...
    FP = 'main/datas/'  # путь к папке со списками слов
    DATA_SOURCE = {
    ...
    for field in DATA_SOURCE.keys():
        if field not in data:
            ...
            words = await work_with_json.read_from_json(folderpath=FP, filename=fn)
    ...
    for field in DATA_SOURCE.keys():
        ...
        if not data[field]:
            fn = DATA_SOURCE[field]
            spisok = await work_with_json.read_from_json(folderpath=FP, filename=fn)
